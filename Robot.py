...
class Robot(object):
    ...
    def angle(self, angle, speed, seconds=.2):
        """Move at the angle specified, given the speed and time."""

        # ensure that the angle is between 0 and 359 degrees
        angle = angle % 360

        if (angle <=90):
            # turning right going forward
            rspeed = int(speed *cos(angle*pi/180))
            lspeed = speed
            self._left.run(Adafruit_MotorHAT.FORWARD)
            self._right.run(Adafruit_MotorHAT.FORWARD)
        elif(angle >=270):
            # turning left going forward
            rspeed = speed
            lspeed = int(speed *cos(angle*pi/180))
            self._left.run(Adafruit_MotorHAT.FORWARD)
            self._right.run(Adafruit_MotorHAT.FORWARD)
        elif(angle >=180):
            # turning left going backward
            rspeed = speed
            lspeed = int(speed *cos(angle*pi/180))
            self._left.run(Adafruit_MotorHAT.BACKWARD)
            self._right.run(Adafruit_MotorHAT.BACKWARD)
        else:
            # turning right going backward
            rspeed = int(speed *cos(angle*pi/180))
            lspeed = speed
            self._left.run(Adafruit_MotorHAT.BACKWARD)
            self._right.run(Adafruit_MotorHAT.BACKWARD)
